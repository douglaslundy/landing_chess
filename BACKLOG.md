...
| 2 | Migrar app atual (landing + checkout + webhook + cron) para Next.js | 🟡 Deploy feito, checkout pendente (2026-09-15) — [design](docs/superpowers/specs/2026-09-15-fase2-app-migration-design.md), [plano](docs/superpowers/plans/2026-09-15-fase2-app-migration.md). `chess.dlsistemas.com.br` serve a landing real (Tasks 1-8 completas e revisadas, migration aplicada, deploy feito). Faltando: usuário cadastrar credenciais reais (Mercado Pago TEST, SMTP, `PRODUCT_ACCESS_URL`, `CRON_SECRET`) no `/opt/xadrez-essencial/.env` da VPS — sem isso `/api/config`, pagamentos, webhook e cron continuam retornando erro. Task 9 (verificação end-to-end) e Task 10 (limpeza dos arquivos Vercel) ficam pendentes até as credenciais serem cadastradas. |
